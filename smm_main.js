...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // --- SMM REFILL COOLDOWN TIMER LOGIC (Sahi wala) ---
    // (Yeh logic `countdown.js` se copy ki gayi hai lekin `smm_history.php` ke buttons ke liye)
    
    const refillButtons = $$('.btn-refill[data-cooldown-ends]');
    
    if (refillButtons.length > 0) {
        
        const interval = setInterval(() => {
            let activeButtons = 0; // Zinda timers ka count

            refillButtons.forEach(btn => {
                const textSpan = btn.querySelector('.refill-button-text');
                if (!textSpan || btn.disabled === false) {
                    return; // Yeh pehle hi enable ho chuka hai
                }

                const endDateString = btn.getAttribute("data-cooldown-ends");
                
                if (!endDateString) {
                    return;
                }
                
                // MySQL ke 'YYYY-MM-DD HH:MM:SS' format ko JS ke liye fix karein
                const compatibleDateString = endDateString.replace(" ", "T");
                const endDateTime = new Date(compatibleDateString).getTime();

                if (isNaN(endDateTime)) {
                    textSpan.innerText = "Refill (Error)";
                    return;
                }
                ...
                if (distance < 0) {
                    // Time poora ho gaya
                    btn.disabled = false;
                    textSpan.innerText = 'Refill';
                    btn.removeAttribute('data-cooldown-ends');
                } else {
                    // Timer abhi chal raha hai
                    activeButtons++;
                    const hours = Math.floor((distance % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60));
                    const minutes = Math.floor((distance % (1000 * 60 * 60)) / (1000 * 60));
                    const seconds = Math.floor((distance % (1000 * 60)) / 1000);
                    ...
                    textSpan.innerText = `Wait ${hours}h ${minutes}m ${seconds}s`;
                }
            });

            if (activeButtons === 0) {
                clearInterval(interval); // Agar koi timer nahi bacha toh interval rok dein
            }
        }, 1000);
    }
    // --- REFILL COOLDOWN LOGIC KHATAM ---
